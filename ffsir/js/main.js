...
function getPoint() {
	...
	if($("#inputname4").html() == '课程名称'||$("#inputname4").html() == '课程名称'||$("#inputname4").attr('gettype')=='ssid'){
	    $("#queryhide").show();
		$('#inputvalue4').attr("disabled", true);
		$('#inputvalue4').attr("placeholder", "点击【查询课程】进行选课。");
		$('#inputname5').parent().parent().hide();
	}else{
	    $("#queryhide").hide();
	}
	if($("#inputname5").html() == '课程信息'||$("#inputname5").html() == '课程名称'||$("#inputname5").attr('gettype')=='ssid'){
		$('#inputname5').attr("disabled", true);
		$('#inputname5').attr("placeholder", "请忽略此项。");
		$('#inputname5').parent().parent().hide();
	}
...
function queryOrder(type,content,page){
	...
	$.ajax({
		...
		success : function(data) {
		    if(data.data ==''){
		        $('#list').append('<tr class="c-table__row"><td class="c-table__cell">未查到，请检查账号/学号是否正确</td></tr>');
		    }
			if(data.code == 0){
				var status;
				$.each(data.data, function(i, item){
					if(item.status==1)
						status='<span class="c-badge c-badge--success c-badge--small">已完成</span>';
					else if(item.status==2)
						status='<span class="c-badge c-badge--small c-badge--fancy">处理中</span>';
					else if(item.status==3)
						status='<span class="c-badge c-badge--small c-badge--danger">异常</span>&nbsp;<button type="submit" class="c-btn c-btn--small" onclick="fillOrder('+item.id+',\''+item.skey+'\')">补交</button>';
					else if(item.status==4)
						status='<font color=red>已退款</font>';
					else
						status='<span class="c-badge c-badge--small c-badge--primary">待处理</span>';
					$('#list').append('<tr class="c-table__row" orderid='+item.id+'><td class="c-table__cell">'+item.input+'</td><td class="c-table__cell">'+item.name+'</td><td class="c-table__cell">'+item.value+'</td><td class="c-table__cell">'+item.addtime+'</td><td class="c-table__cell">'+status+'</td><td class="c-table__cell"><a href="javascript:showOrder('+item.id+',\''+item.skey+'\')" title="查看订单详细" class="c-badge c-badge--info c-badge--small">详细</a></td></tr>');
					if(item.result!=null){
						if(item.status==3){
				...
				var addstr = '';
				if(data.islast==true) addstr += '<button class="c-btn c-btn--small feather icon-chevron-left pull-left" onclick="queryOrder(\''+data.type+'\',\''+data.content+'\','+(data.page-1)+')">上一页</button>';
				if(data.isnext==true) addstr += '<button class="c-btn c-btn--small feather icon-chevron-right pull-right" onclick="queryOrder(\''+data.type+'\',\''+data.content+'\','+(data.page+1)+')">下一页</button>';
				$('#list').append(''+addstr+'</td></tr>');
